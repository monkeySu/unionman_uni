...
window.BMAP_DRAWING_MARKER = BMAP_DRAWING_MARKER;
window.BMAP_DRAWING_POLYLINE = BMAP_DRAWING_POLYLINE;
window.BMAP_DRAWING_CIRCLE = BMAP_DRAWING_CIRCLE;
window.BMAP_DRAWING_RECTANGLE = BMAP_DRAWING_RECTANGLE;
window.BMAP_DRAWING_POLYGON = BMAP_DRAWING_POLYGON;
...
(function () {
    ...
    var DrawingManager =
         ...
         * <br />"<b>markerOptions</b>" : {CircleOptions} 所画的点的可选参数，参考api中的<a href="http://developer.baidu.com/map/reference/index.php?title=Class:总类/覆盖物类">对应类</a>
         * <br />"<b>circleOptions</b>" : {CircleOptions} 所画的圆的可选参数，参考api中的<a href="http://developer.baidu.com/map/reference/index.php?title=Class:总类/覆盖物类">对应类</a>
         * <br />"<b>polylineOptions</b>" : {CircleOptions} 所画的线的可选参数，参考api中的<a href="http://developer.baidu.com/map/reference/index.php?title=Class:总类/覆盖物类">对应类</a>
         * <br />"<b>polygonOptions</b>" : {PolygonOptions} 所画的多边形的可选参数，参考api中的<a href="http://developer.baidu.com/map/reference/index.php?title=Class:总类/覆盖物类">对应类</a>
         * <br />"<b>rectangleOptions</b>" : {PolygonOptions} 所画的矩形的可选参数，参考api中的<a href="http://developer.baidu.com/map/reference/index.php?title=Class:总类/覆盖物类">对应类</a>
         *
         * @example <b>参考示例：</b><br />
    ...
    DrawingManager.prototype._initialize = function (map, opts) {
        ...
        DrawingManager.prototype._open = function () {
            ...
            this._mask.addEventListener('remove', function() {
              if (mask.container) {
                mask.container.removeEventListener('touchstart', mask.container.ontouchstart);
                mask.container.removeEventListener('touchmove', mask.container.ontouchmove);
                mask.container.removeEventListener('touchend', mask.container.ontouchend);
              }
            });
        }
    ...
    // 将移动端 touch 相关函数转换为 mouse 函数
    function touchEventToMouseEvent(event, eventType) {
      var isTouchend = event.type === 'touchend';
      var mapMask = document.querySelector('.BMap_mask');
      var mapMaskBoundingClient = mapMask ? mapMask.getBoundingClientRect() : {};

      if (!event || !event.targetTouches || (event.targetTouches.length != 1 && !isTouchend)) {
        return null;
      }

      var te = isTouchend ? event.changedTouches[0] : event.targetTouches[0];
      var clientX = te.clientX,
        clientY = te.clientY,
        offsetX = clientX - (mapMaskBoundingClient.x || 0),
        offsetY = clientY - (mapMaskBoundingClient.y || 0);

      var simEvent = new MouseEvent(eventType, {
          clientX: clientX,
          clientY: clientY,

          // hack: MouseEvent 没有 offsetX/Y 相关参数，而后面获取点的时候需要用到，所以这里使用 screenX/Y 来代替
          screenX: offsetX,
          screenY: offsetY,
          button: 0,
          buttons: 0,
    ...
    // 替换指定元素的 touch 事件，改为触发相应的 mouse 事件
    function findElm(targetElement) {
      if (!targetElement) {
        return;
      }

      targetElement.ontouchstart = function (e) {
      ...
      targetElement.ontouchmove = function (e) {
        e.preventDefault();
      ...
      targetElement.ontouchend = function (e) {
      ...
      targetElement.addEventListener('touchstart', targetElement.ontouchstart);

      targetElement.addEventListener('touchmove', targetElement.ontouchmove);

      targetElement.addEventListener('touchend', targetElement.ontouchend);
    }
    ...
    Mask.prototype.initialize = function (map) {
        ...
        div.style.cssText = "position:absolute;background:url(about:blank);cursor:crosshair;width:" + size.width +
    ...
    Mask.prototype._bind = function () {
        ...
        var domEvent = function (e) {
            var type = e.type;
            e = baidu.getEvent(e);
            // console.log('me...', me.getDrawPoint(e));
            ...
            if (type == "click") {
                ...
                if (Math.abs(nowXY.x - lastMousedownXY.x) < 5 && Math.abs(nowXY.y - lastMousedownXY.y) < 5) {
                    if (!lastClickXY || !(Math.abs(nowXY.x - lastClickXY.x) < 5 && Math.abs(nowXY.y -
                            lastClickXY.y) < 5)) {
                        dispatchEvent('click');
    ...
    Mask.prototype.getDrawPoint = function (e) {
      var isMobileDev = isMobile();
        var map = this._map,
            ...
            // 如果是移动设备，则使用 hack 的 screenX/Y 属性
            x = isMobileDev ? e.screenX : e.offsetX || e.layerX || 0,
            y = isMobileDev ? e.screenY : e.offsetY || e.layerY || 0;
    ...
    DrawingTool.prototype._generalHtml = function (map) {
        ...
        var getItem = function (className, drawingType) {
            return '<a class="' + className + '" drawingType="' + drawingType +
                '" href="javascript:void(0)" title="' + tips[drawingType] + '" onfocus="this.blur()"></a>';
        }
