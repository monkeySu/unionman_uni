...
;(function () {
  ...
  var MarkerClusterer =
     ...
     *    !! 改为marker后，styles样式参考markerOptions
     *
     *    labelStyles {IconStyle} 自定义聚合文本样式，请参考Label类<br />
     */
    (BMapLib.MarkerClusterer = function (map, options) {
      ...
      this._labelStyles = opts["labelStyles"] || {}
  ...
  /**
   * 获取聚合的文字样式风格集合
   * @return {IconStyle} 聚合的文字风格集合
   */
  MarkerClusterer.prototype.getLabelStyles = function () {
  ...
  /**
   * 设置聚合的文本样式风格集合
   * @param {Array<IconStyle>} styles 文本样式风格数组
   * @return 无返回值
   */
  MarkerClusterer.prototype.setLabelStyles = function (styles) {
  ...
  function Cluster(markerClusterer) {
    ...
    // 改为marker, 方便图标样式调整
    const styles = this._markerClusterer.getStyles()
    this._clusterMarker = new BMap.Marker(
      new BMap.Point(0, 0),
      Object.assign(
        {},
        {
    ...
    this._label = new BMap.Label(this._markers.length.toLocaleString())
    this._label.setStyle(
      Object.assign({}, this._markerClusterer.getLabelStyles())
    )

    this._clusterMarker.setLabel(this._label)

    this._label.disableMassClear()
  ...
  Cluster.prototype.updateClusterMarker = function () {
    ...
    this._label.setContent(this._markers.length.toLocaleString())
    this._label.disableMassClear()
    ...
    this._clusterMarker.addEventListener("click", (event) => {
      // 点击聚合点时，更改为放大一级视图（取消原有行为）
      // thatMap.setViewport(thatBounds)
      // 浏览器不同行为event不一样
      const point = event.point || event.target.point
      thatMap.setCenter(point)
      thatMap.zoomIn()
    })
  }
